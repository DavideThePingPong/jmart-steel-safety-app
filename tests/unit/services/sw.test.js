...
describe('Service Worker (sw.js)', () => {
  ...
  describe('cache constants', () => {
    it('should define CACHE_VERSION as v72', () => {
      expect(env.CACHE_VERSION).toBe('v72');
    });

    it('should name caches with version suffix', () => {
      expect(env.STATIC_CACHE).toBe('jmart-static-v72');
      expect(env.DYNAMIC_CACHE).toBe('jmart-dynamic-v72');
      expect(env.CDN_CACHE).toBe('jmart-cdn-v72');
    });
  });
  ...
  describe('CLEAR_CACHE message handler', () => {
    it('should only delete caches NOT matching current version [REGRESSION]', async () => {
      ...
      // Current v72 caches MUST be preserved
      expect(deleted).not.toContain('jmart-static-v72');
      expect(deleted).not.toContain('jmart-cdn-v72');
      expect(deleted).not.toContain('jmart-dynamic-v72');
    });

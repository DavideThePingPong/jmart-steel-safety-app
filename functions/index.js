...
const PRESTART_PROMPT_VERSION = "v3-bucketed";
...
- AS 4100 (Steel structures — design + bolted connection torque sequencing).
- AS/NZS 1554 (Structural steel welding — process, qualifications, NDT categories).
- AS/NZS 1891 (Industrial fall-arrest systems and devices) — anchor/lanyard/harness specs for >2 m work.
- AS 1657 (Fixed platforms, walkways, stairways, ladders).
- AS/NZS 4576 (Scaffolding — guidance and use).
- AS 2550 (Cranes — safe use).
...
// When the model returns a flat-string methodology (instead of the structured
// object), bucket each bullet into PRE-TASK / Engineer / Admin / PPE / QA /
// Emergency using keyword heuristics. Models reliably produce flat bullets
// with task-relevant content; the categorisation is deterministic from there.
function bucketFlatMethodology(flatText) {
  if (typeof flatText !== "string" || !flatText.trim()) {
    return { preTask: [], controlsEngineer: [], controlsAdmin: [], controlsPPE: [], qaHoldPoint: [], emergency: [] };
  }
  ...
  const score = (line) => {
    ...
    // QA hold points — torque marks, weld inspection, sign-offs
    if (/witness|paint mark|torque mark|qa hold|hold point|engineer sign-?off|nDT|weld inspection/i.test(line)) return "qaHoldPoint";
    // Emergency
    if (/emergency|warden|muster|first[- ]aid|nearest hospital|evacuat/i.test(l)) return "emergency";
    // PPE — explicit kit
    if (/\bPPE\b|hard hat|hi-?vis|safety boot|gloves|safety glass|eye protect|hearing protect|respirator|harness|lanyard/i.test(line)) return "controlsPPE";
    // Engineering controls — physical
    if (/edge protection|fall arrest|anchor|guardrail|screen|guarding|isolat|barrier|exclusion zone|fence/i.test(l)) return "controlsEngineer";
    // Pre-task — setup, verification, sign-on
    if (/sign[- ]on|swms|induct|calibrat|verify|inspect.*before|brief|toolbox|pre-?start|tag.*before|in-?date/i.test(l)) return "preTask";
    // Admin controls — process, sequencing, watchers
    if (/spotter|watch\b|watcher|two-?person|sequence|coordinate|report.*to|do not|forbid|prohibited|fire watch|hot works permit|permit/i.test(l)) return "controlsAdmin";
    // QA fallback for "torque" / "mark" if not caught above
    if (/torque|paint|mark each|connection.*before/i.test(l)) return "qaHoldPoint";
    // Final fallback — admin (process control)
    return "controlsAdmin";
  };

  const buckets = { preTask: [], controlsEngineer: [], controlsAdmin: [], controlsPPE: [], qaHoldPoint: [], emergency: [] };
  for (const b of bullets) {
    buckets[score(b)].push(b);
  }
  return buckets;
}
...
function parseAutofillJson(content) {
  ...
  // Methodology: ideal case is a structured object the model returned. In
  // practice Kimi K2.6 keeps producing flat bullet strings even with explicit
  // schema in the system prompt. So we ALSO accept a flat string and run a
  // keyword bucketer to re-shape it into the labelled-section format. That
  // way the user always gets the polished output regardless of whether the
  // model honoured the schema.
  let methodology;
  if (parsed.methodology && typeof parsed.methodology === "object" && !Array.isArray(parsed.methodology)) {
    ...
    methodology = formatMethodology(bucketFlatMethodology(parsed.methodology));
  } else {
    methodology = "";
  }
